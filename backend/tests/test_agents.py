...
async def test_price_agent_returns_valid_envelope(TEST_PRODUCT):
    with patch("backend.agents.price_agent.call_agent", new_callable=AsyncMock) as mock_call:
...
async def test_specs_agent_returns_valid_envelope(TEST_PRODUCT):
    with patch("backend.agents.specs_agent.call_agent", new_callable=AsyncMock) as mock_call:
...
async def test_review_agent_returns_valid_envelope(TEST_PRODUCT):
    with patch("backend.agents.review_agent.call_agent", new_callable=AsyncMock) as mock_call:
